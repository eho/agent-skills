...
For scaffolds that include gluestack or `react-native-svg`, make the native Metro bundle check explicit. From the app root:

```sh
npx expo export --platform ios --output-dir dist-ios-check
```

With Bun from a monorepo root, prefer:

```sh
bun run --cwd apps/mobile expo export --platform ios --output-dir dist-ios-check
```

Before running the export, inspect the mobile package manifest using `package-installation.md`. Runtime dependencies must live in the app package that owns the Metro graph. If a resolver error appears, classify it before adding packages: direct app import, official peer/setup requirement, wrong workspace install location, or verified undeclared runtime import. For affected `react-native-svg` releases that import `buffer` without declaring it, `buffer@^6.0.3` is a narrow workaround in the Expo app dependencies. Treat unresolved native export errors as an incomplete scaffold and repair them before the final report.
...
- Remove `.expo/`, `.expo-shared/`, `dist/`, `dist-*`, `build/`, `.turbo/`, temporary EAS inspect output, copied package-manager caches, and native `ios/` or `android/` directories unless the user requested committed native projects.
...
- Ensure `.gitignore` covers `.expo/`, `.expo-shared/`, `node_modules/`, native build output, web `dist/`, `dist-*` verification output, EAS/local build output, logs, and environment files while not ignoring committed source, config, docs, or intended assets.
