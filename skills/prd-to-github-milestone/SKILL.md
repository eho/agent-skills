...
1. **Setup Labels**: Before creating any issues, verify the `user-story` label exists. Run `gh label list --limit 1000 | grep "user-story"`. If not found, create it: `gh label create "user-story" --color "0e8a16" --description "User story task"`. This prevents failures when assigning labels during issue creation.
2. **Parse PRD**: Read the specified PRD file (e.g., `docs/PRD.md` or `tasks/prd-[feature].md`). Extract all User Stories and their complete details, including Titles, Descriptions, Acceptance Criteria, Technical Notes, Data Models, dependencies, and any other relevant context.
...
5. **Create Issues**: Loop through the extracted stories. For each uncreated story, construct a GitHub blob URL to the PRD file. Get the repo info with `gh repo view --json nameWithOwner -q` (format: `owner/repo`), then format the issue body:
   ...
   Run `gh issue create --title "<Story ID>: <Title>" --body "<Formatted Body>" --label "user-story"` and capture the issue number. Since the command outputs a URL (e.g., `https://github.com/owner/repo/issues/42`), extract the issue number:
   ```bash
   ISSUE_URL=$(gh issue create --title "..." --body "..." --label "user-story")
...
*Action:*
1. Setup labels: `gh label list --limit 1000 | grep "user-story"`. If not found, run `gh label create "user-story" --color "0e8a16"`.
2. Read `tasks/prd-login.md`.
3. Extract US-001 (Login), US-002 (Logout) with dependencies: US-002 depends on US-001.
4. Get repo info: `gh repo view --json nameWithOwner -q` → returns `myorg/myapp`.
5. Create issue for US-001:
   ```bash
   ISSUE_URL=$(gh issue create --title "US-001: User Login" \
     --body "## Description
   ...
   ```
6. Create issue for US-002 (with dependency noted):
   ```bash
   ISSUE_URL=$(gh issue create --title "US-002: User Logout" \
     --body "## Description
...
8. Output summary:
   ...
   | Story ID | Title | Issue # | URL |
   |----------|-------|---------|-----|
   | US-001 | User Login | #42 | https://github.com/myorg/myapp/issues/42 |
   | US-002 | User Logout | #43 | https://github.com/myorg/myapp/issues/43 |
   ```