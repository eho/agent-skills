...
| [**Design to Issues**](skills/design-to-issues/SKILL.md) | `/design-to-issues` | Idempotently synchronize a revised design document's stories, acceptance criteria, dependencies, labels, and milestone with GitHub Issues. |
| [**Feature Delivery**](skills/feature-delivery/SKILL.md) | `/feature-delivery` | Orchestrate fresh or resumed feature delivery by reconciling GitHub state, delivering dependency-ordered stories one at a time, and running the final audit. |
| [**User Story Delivery**](skills/user-story-delivery/SKILL.md) | `/user-story-delivery` | Coordinate independent implementation and review for one story, resuming its existing PR when needed and bounding review-fix loops. |
| [**User Story Implementer**](skills/user-story-implementer/SKILL.md) | `/user-story-implementer` | Implement or resume one issue with a clean synchronized default-branch base, verified dependencies, focused tests, and one story PR. |
...
**4. Deliver or resume the feature** — Use `/feature-delivery` for the full design-to-release workflow from a revised design document. It synchronizes issues, reconciles completed, in-progress, blocked, deferred, and remaining stories from live GitHub state, resumes existing PRs before starting new branches, runs `/user-story-delivery` one story at a time, and finishes with `/post-implementation-reviewer`.

**5. Synchronize stories with GitHub manually when needed** — Use `/design-to-issues` directly to create or refresh agent-ready GitHub Issues. It reconciles revised story bodies and acceptance criteria exactly, upserts dependency comments without duplicates, creates only missing labels and milestones, and reports removed or deferred stories without closing or deleting anything.

**6. Implement and review one story manually when needed** — Use `/user-story-delivery` directly for the full loop around a single issue. It runs `/user-story-implementer`, hands the resulting PR to `/user-story-reviewer`, addresses requested changes or reviewer-fixed small issues, and repeats review up to a bounded limit until the PR is merged, awaiting a required human merge, or blocked. Use `/user-story-implementer` or `/user-story-reviewer` directly when you only want one half of the workflow.
